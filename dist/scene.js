...
if (renderer) {
  ...
  let visible = true, frame = 0, lastTime = 0, elapsed = 0, progress = 0;
  ...
  function start() { if(!frame) {lastTime=0;frame=requestAnimationFrame(render);} }
  function stop() { cancelAnimationFrame(frame); frame=0; lastTime=0; }
  function resize() {
    const width=host.clientWidth,height=host.clientHeight;
    if(!width || !height) return;
    renderer.setSize(width,height,false); camera.aspect=width/height; camera.updateProjectionMatrix(); start();
  }
  ...
  addEventListener('portfolio:motion', event => {paused=event.detail.paused; if(paused) stop(); start();});
  addEventListener('portfolio:progress', event => {progress=event.detail.phase;if(visible && !paused) start();});
  ...
  canvas.addEventListener('webglcontextrestored', () => {canvas.hidden=false;resize();});
  resize();
}
